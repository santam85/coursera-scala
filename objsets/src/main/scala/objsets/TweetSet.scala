...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedAcc(acc: Tweet): Tweet = {
    filter((p: Tweet) => { p.retweets > acc.retweets}).mostRetweetedAcc(elem)
  }
...
object GoogleVsApple {
  ...
  def filterTweetDataByTermList(l:List[String]):TweetSet = {
    TweetReader.allTweets.filter((p: Tweet) => {
      l.exists((t:String) => {
        p.text.contains(t)
      })
    })
  ...
  lazy val googleTweets: TweetSet = filterTweetDataByTermList(google)

  lazy val appleTweets: TweetSet = filterTweetDataByTermList(apple)
